...
{
    // Verifique se a relação affiliates não é nula e, caso seja, retorne 0
    return $this->affiliates()->exists() ? $this->affiliates->sum(function ($affiliate) {
        return $affiliate->deposits()->where('status', '1')->sum('amount');
    }) : 0;
}

public function getAffiliateDepositCountAttribute(): int
{
    // Conta o número de depósitos feitos pelos afiliados do usuário que o convidou
    return $this->affiliates()->with('deposits')->get()->sum(function ($affiliate) {
        return $affiliate->deposits()->where('status', '1')->count();
    });
}
...
public function getTotalWithdrawalsAttribute()
{
    // Verifique se a relação affiliates não é nula e, caso seja, retorne 0
    return $this->affiliates()->exists() ? $this->affiliates->sum(function ($affiliate) {
        return $affiliate->withdrawals()->where('status', '1')->sum('amount');
    }) : 0;
}
...
public function getAffiliatesCountAttribute()
{
