...
trait DigitoPayTrait
{
    ...
    public function requestQrcodeDigito($request)
    {
        try {
            ...
            if ($response->successful()) {
                $responseData = $response->json();
                $token = self::generateTransactionDigito($responseData['id'], $request->input("amount"), $request->input('accept_bonus'));

                self::generateDepositDigito($responseData['id'], $request->input("amount"));
                return response()->json(['status' => true, 'idTransaction' => $responseData['id'], 'qrcode' => $responseData['pixCopiaECola'],'token' => $token]);
            }
                ...
                if ($wallet->increment('balance', $transaction->price)) {
                    if ($transaction->update(['status' => 1])) {
                        ...
                        if (!empty($deposit)) {
                            ...
                            if ($deposit->update(['status' => 1, 'paid_at' => now()])) {
    ...
    private static function generateTransactionDigito($idTransaction, $amount, $accept_bonus = false)
    {
        $setting = Core::getSetting();
